...
class MonoVideoOdometery(object):
    ...
    def hasNextFrame(self):
        ...
        return self.id < len(self.shot_frames) 
    ...
    def visual_odometery(self):
        ...
        if self.id < 2:
            ...
            absolute_scale = 10 #self.get_absolute_scale()
        ...
        #print(self.R)
        #print("---")
        #print(self.rotationMatrixToEulerAngles(self.R))

        self.xyz_euler = self.rotationMatrixToEulerAngles(self.R)
    ...
    def process_frame(self):
        ...
        if self.id < 2:
            ...
            self.old_frame = self.shot_frames[0]
            self.current_frame = self.shot_frames[1]
            self.visual_odometery()
            self.id = 2
        else:
            self.old_frame = self.current_frame
            #self.current_frame = cv2.imread(self.file_path + str(self.id).zfill(6)+'.png', 0)
            self.current_frame = self.shot_frames[self.id]
            self.visual_odometery()
            self.id += 1