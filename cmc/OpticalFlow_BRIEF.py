...
class OpticalFlow_BRIEF(object):
    ...
    def run(self):
        ...
        for i in range(1, len(frames_np)):
            ...
            for block_idx in range(0, len(block_coordinates_np)):
                ...
                kp_prev_list, kp_curr_list = self.getBRIEFMatches(prev_frame[start_y:end_y, start_x:end_x],
    ...
    def getBRIEFMatches(self, frame1, frame2):
        kp_curr_list = []
        kp_prev_list = []
        ...
        kp_prev = star.detect(frame1, None)
        kp_curr = star.detect(frame2, None)

        # compute the descriptors with BRIEF
        kp_prev, descriptor_prev = brief.compute(frame1, kp_prev)
        kp_curr, descriptor_curr = brief.compute(frame2, kp_curr)
