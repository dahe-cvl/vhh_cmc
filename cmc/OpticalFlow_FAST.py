...
class OpticalFlow_FAST(object):
    ...
    def run(self):
        ...
        for i in range(1, len(frames_np)):
            ...
            for block_idx in range(0, len(block_coordinates_np)):
                ...
                kp_prev_list, kp_curr_list = self.getFASTMatches(prev_frame[start_y:end_y, start_x:end_x],
    ...
    def getFASTMatches(self, frame1, frame2):
        kp_curr_list = []
        kp_prev_list = []

        fast = cv2.FastFeatureDetector_create()
        kp_prev, descriptor_prev = fast.detectAndCompute(frame1, None)
        kp_curr, descriptor_curr = fast.detectAndCompute(frame2, None)
